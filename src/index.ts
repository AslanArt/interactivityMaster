...
import { requestPermissionToEndTool } from './tool-definitions/request-permission-to-end.js';
...
const allToolCapabilities: ToolCapabilitiesStructure = {
  request_user_input: requestUserInputTool.capability,
  request_permission_to_end: requestPermissionToEndTool.capability,
  start_intensive_chat: intensiveChatTools.start.capability,
  ask_intensive_chat: intensiveChatTools.ask.capability,
};

// Conditionally add 'stop_intensive_chat' if it's defined
if (intensiveChatTools.stop) {
  allToolCapabilities.stop_intensive_chat = intensiveChatTools.stop.capability;
}
...
const argv = yargs(hideBin(process.argv))
  ...
  .option('disable-tools', {
    ...
    type: 'string',
    description:
      'Comma-separated list of tool names to disable. Available options: request_user_input, request_permission_to_end, intensive_chat (disables all intensive chat tools).',
    default: '',
  })
  .help()
  .alias('help', 'h')
...
if (isToolEnabled('request_permission_to_end')) {
  // Use properties from the imported tool object
  server.tool(
    'request_permission_to_end',
    typeof requestPermissionToEndTool.description === 'function'
      ? requestPermissionToEndTool.description(globalTimeoutSeconds)
      : requestPermissionToEndTool.description,
    requestPermissionToEndTool.schema,
    async (args) => {
...
// No need to register 'stop_intensive_chat' as it's meant to be disabled for the AI

// --- Final server setup ---
const transport = new StdioServerTransport();
server.connect(transport);

console.error('Interactive MCP server started.');
